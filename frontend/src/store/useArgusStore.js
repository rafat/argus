...
export const useArgusStore = create((set, get) => ({
  ...
  fetchDocuments: async () => {
    // For now, since we only upload documents, we can fetch documents or list them.
    // If there is no specific endpoint, we can manage documents state locally
    // or set a default empty state.
  },
  ...
  uploadDocument: async (file) => {
    ...
    try {
      ...
      const docClaims = response.data.claims;
